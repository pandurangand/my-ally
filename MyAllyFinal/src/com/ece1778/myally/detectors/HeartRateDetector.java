...
public class HeartRateDetector {
	...
	private static HeartRateListener _listener;

	public HeartRateDetector(PowerManager powerManager, 
	...
	public void init() {
		...
		if(Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
			_surfaceView.getHolder().setType(
					SurfaceHolder.SURFACE_TYPE_PUSH_BUFFERS);
		}


		//PowerManager pm = (PowerManager) getSystemService(Context.POWER_SERVICE);
		wakeLock = _powerManager.newWakeLock(
				PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "DoNotDimScreen");
	}	
	
	public void setListener(HeartRateListener listener) {
		_listener = listener;
	}
	...
	private static class HRPreviewCallback implements PreviewCallback {
		...
		private static final AtomicBoolean _processing = 
		...
		public void onPreviewFrame(byte[] data, Camera cam) {			
			...
			if (totalTimeInSecs >= 5) {
				...
				_startTime = System.currentTimeMillis();
				_beats = 0;
				_listener.onHeartRateChange(beatsArrayAvg / beatsArrayCnt);
			}
			_processing.set(false);
		}
	}
	...
	private static class HRSurfaceCallback implements SurfaceHolder.Callback {
		...
		public void surfaceCreated(SurfaceHolder holder) {
			try {
				...
				Log.e("HeartRateDetector-surfaceCallback", "Exception in " +
						"setPreviewDisplay()", t);
			}
		}
		...
		public void surfaceChanged(SurfaceHolder holder, int format, int width, 
			...
			if (size != null) {
				...
				Log.d(TAG, "Using width=" + size.width + 
						" height=" + size.height);
			}
